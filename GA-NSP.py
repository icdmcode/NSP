...
def calculate_support(pattern, dataset_size):
    ...
    if len(nsc) == 1:
        ...
        # 目前暂时忽略m>=2的情况（较为复杂但意义应该不大）
        # 即这里需要补全Algorithm 2当中最后一个else分支
        ########## 有待补全过来
        """
    ...
    def mutate(self, mutation_rate):
        ...
        for itemset in self.sequence:
            if random.random() < mutation_rate:
                # 目前采用随机选择一个项进行肯定和否定的转换（论文当中还介绍了其他变异策略）
                # print("变异")
...
# 遗传算法
def genetic_algorithm(psp_list, min_sup, population_size=500, num_generations=400, mutation_rate=0.05, crossover_rate=0.6, k=50):
...
if __name__ == "__main__":
    ...
    file_path = 'test/psp_list_MSNBC_0.1.csv'
    ...
    # min_sup = 2  # 设置最小支持度阈值
    min_sup = math.ceil(len(dataset) * 0.1)  # 向上取整
